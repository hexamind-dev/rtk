...
pub fn run(
    ...
    if dry_run {
        ...
        let consent_given = cfg.telemetry.consent_given == Some(true) && cfg.telemetry.enabled;
        if env_disabled || !consent_given {
            println!("  [info] Anonymous telemetry is disabled (consent required: rtk telemetry enable or accept during rtk init)");
        } else {
            println!("  [info] Anonymous telemetry is enabled (rtk telemetry disable or RTK_TELEMETRY_DISABLED=1 to opt out)");
        }
...
fn prompt_telemetry_consent() -> Result<()> {
    ...
    eprintln!("  Details: https://github.com/hexamind-dev/rtk/blob/master/docs/TELEMETRY.md");
    eprintln!();
